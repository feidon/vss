...
**Architecture:** Two Alembic migration files (schema + seed) replace the current startup logic. Docker entrypoint runs `alembic upgrade head` before uvicorn. Test fixtures keep using `metadata.create_all()` with a new seed helper.
...
|------|--------|----------------|
| `backend/infra/seed.py` | Modify | Switch uuid7 -> uuid5 with fixed namespace |
...
| `backend/infra/postgres/alembic/versions/001_create_schema.py` | Create | DDL for all 7 tables |
...
### Task 1: Make Seed IDs Deterministic (uuid7 -> uuid5)

**Files:**
- Modify: `backend/infra/seed.py:23,32-47`
- Test: `backend/tests/infra/test_seed.py`

- [ ] **Step 1: Run existing seed tests to confirm green baseline**

Run: `cd /home/feidon/Documents/vss/backend && uv run pytest tests/infra/test_seed.py -v`
Expected: All tests PASS

- [ ] **Step 2: Update `infra/seed.py` to use uuid5 with a fixed namespace**

Replace the uuid7 imports and ID generation at the top of the file:
...
PLATFORM_ID_BY_NAME = {
    name: uuid5(_NS, name) for name in ["P1A", "P1B", "P2A", "P2B", "P3A", "P3B"]
}
...
VEHICLE_ID_BY_NAME = {name: uuid5(_NS, name) for name in ["V1", "V2", "V3"]}
```

- [ ] **Step 3: Run seed tests to verify they still pass**

Run: `cd /home/feidon/Documents/vss/backend && uv run pytest tests/infra/test_seed.py -v`
Expected: All tests PASS (uuid5 is deterministic across calls, so `TestDeterministicIds` passes)

- [ ] **Step 4: Run full unit test suite to check nothing breaks**
...
```bash
cd /home/feidon/Documents/vss && git add backend/infra/seed.py
git commit -m "refactor: use deterministic uuid5 for seed IDs"
```

---

### Task 2: Move psycopg to Production Dependencies
...
### Task 3: Configure Alembic env.py
...
### Task 4: Create Schema Migration (001)

**Files:**
- Create: `backend/infra/postgres/alembic/versions/001_create_schema.py`
...
def upgrade() -> None:
    # Drop existing tables for clean slate (reverse FK order).
    for name in [
        "services",
        "node_layouts",
        "node_connections",
    ...
    op.create_table(
        ...
        sa.Column("name", sa.String, nullable=False),
        sa.Column(
            "station_id", sa.Uuid, sa.ForeignKey("stations.id"), nullable=False
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column("name", sa.String, nullable=False),
        sa.Column(
            "vehicle_id", sa.Uuid, sa.ForeignKey("vehicles.id"), nullable=False
        ),
        sa.Column("route", JSONB, nullable=False),
        sa.Column("timetable", JSONB, nullable=False),
    )

    op.create_table(
        "node_connections",
        sa.Column("from_id", sa.Uuid, primary_key=True),
        sa.Column("to_id", sa.Uuid, primary_key=True),
    )
...
- [ ] **Step 3: Verify migration can be detected by Alembic**

Run: `cd /home/feidon/Documents/vss/backend && DATABASE_URL=postgresql+asyncpg://vss:vss@localhost:5432/vss uv run alembic heads`
Expected: Shows `001` as the head revision

- [ ] **Step 4: Commit**

```bash
cd /home/feidon/Documents/vss && git add backend/infra/postgres/alembic/versions/
git commit -m "feat: add Alembic migration 001 for schema creation"
```

---

### Task 5: Create Seed Data Migration (002)

**Files:**
- Create: `backend/infra/postgres/alembic/versions/002_seed_data.py`

- [ ] **Step 1: Create the seed data migration file**
...
- [ ] **Step 2: Verify Alembic sees both migrations in order**
...
### Task 6: Create Test Seed Helper and Update API Fixtures
...
### Task 7: Remove Old Startup Logic and Delete infra/postgres/seed.py
...
### Task 8: Update Dockerfile Entrypoint
...
### Task 9: Integration Test — Alembic Round-Trip
...
### Task 10: Update README TODO List
