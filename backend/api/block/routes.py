...
async def get_block(block_id: UUID, block_service: BlockAppService = Depends(get_block_service)):
    try:
...
async def update_block(block_id: UUID, request: UpdateBlockRequest, block_service: BlockAppService = Depends(get_block_service)):
    try:
